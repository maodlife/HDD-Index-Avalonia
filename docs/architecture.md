...
- 没有多文件事务回滚、自动备份、应用内损坏文件恢复、撤销或重做机制。当前维护者通过 GitHub 仓库保存 JSON，并以已提交、已推送的 Git 历史作为版本恢复手段，因此自动备份与应用内恢复暂缓。
...
- 配置或 Repository 损坏时只提供诊断、重试和数据目录重选；从 Git 历史或手工备份恢复仍需在应用外完成。
