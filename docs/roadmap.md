...
- **暂缓**：已经完成当前必要范围，其余方向有明确价值，但现阶段已有足够的外部方案或缺少实际需求，因此暂不实施。
...
| P3 | 数据安全与恢复 | 暂缓 | P3.1 逐文件原子保存已完成；当前使用 GitHub 仓库保存 JSON，备份与应用内恢复暂不实施 |
...
- `v1.1` 和 `v1.2` 已通过自动发布流程成功发布。
- 应用、测试、CI 和 Windows 发布链路已迁移到 .NET 10 LTS。
...
**状态：暂缓（2026-08-15 开始；P3.1 于 2026-08-19 完成；其余切片于 2026-08-20 暂缓）**
...
P3 拆成以下可独立验证的安全层级；当前只实施 P3.1：

| 切片 | 主题 | 状态 | 目标 |
| --- | --- | --- | --- |
| P3.1 | 逐文件原子保存 | 已完成 | 同目录写临时文件、刷新到磁盘后原子替换；失败时保留原文件 |
| P3.2 | 受控备份 | 暂缓 | 在不让大索引备份无限增长的前提下保留可回退版本 |
| P3.3 | 应用内恢复 | 暂缓 | 识别主文件、备份和临时文件状态，并由用户确认恢复来源 |
...
- 当前维护者将配置、Repository 和 File Tree JSON 保存在 GitHub 仓库中，并通过提交和推送保留版本历史；这套外部 Git 历史继续作为当前版本恢复手段。
- 因此 P3.2、P3.3 现阶段的收益不足以覆盖新增存储策略、恢复选择和 UI 复杂度，暂不实施。若未来出现不使用 Git 的主要用户、实际恢复成本升高，或需要应用自动处理未提交数据，再重新规划这两个切片。
- GitHub 只保护已经提交并推送的版本；尚未提交的本地修改仍只受 P3.1 的逐文件原子保存保护。
...
- 备份选择和损坏文件恢复属于已暂缓的 P3.2/P3.3；若后续重新启动，应复用同一启动诊断契约。
...
- **自动备份与应用内恢复**：当前维护者使用 GitHub 仓库保存 JSON，已提交并推送的数据可以通过 Git 历史恢复；等非 Git 使用场景或真实恢复成本出现后再重启 P3.2/P3.3。
- **跨平台发布**：当前核心使用场景和发布链路都是 Windows，且打开文件夹功能直接依赖 Windows Explorer。
- **撤销/重做**：当前没有高于数据可靠性和发布维护的实际需求。
