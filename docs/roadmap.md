...
| P5 | 应用编排边界重构 | 实施中 | P5.1 文件扫描边界已完成；下一切片为 P5.2 外部交互与组合根 |
...
**状态：实施中（2026-07-28 开始；P5.1 已完成）**

P5.1 已将文件扫描从 `FileNode` 提取到独立边界。当前剩余的主要压力集中在
`MainWindowViewModel`：它仍负责服务组合、持久化、对话框、平台调用和大量命令编排；
