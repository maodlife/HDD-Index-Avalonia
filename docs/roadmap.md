...
- 路线图只承诺 P 级目标；备份数量、恢复交互等产品决策在对应阶段开始时再确定。
...
| P3 | 数据安全与恢复 | 待规划，下一阶段 | 逐文件原子保存、有限备份、可解释的故障与恢复路径 |
| P4 | 首次启动与启动故障恢复 | 候选，依赖 P3 | 新用户可直接启动，已有用户遇到损坏数据时不会被静默覆盖 |
| P5 | 应用编排边界重构 | 候选 | 提取扫描和平台边界，降低 `MainWindowViewModel` 与 Model 的职责压力 |
...
下一次用户版本暂定在 P3 完成后发布，版本号暂定为 `v1.2`。
...
P4 应复用 P3 的备份与恢复原语，避免分别实现两套数据恢复逻辑。
