...
| P4 | 首次启动与启动故障恢复 | 待合入 | P4.1 至 P4.3 已实现；首次设置、路径修复、诊断和单索引隔离正在集成 |
| P3 | 数据安全与恢复 | 实施中 | P3.1 逐文件原子保存已实现；备份与应用内恢复仍为后续候选 |
...
| 切片 | 主题 | 状态 | 目标 |
| --- | --- | --- | --- |
| P3.1 | 逐文件原子保存 | 待合入 | 同目录写临时文件、刷新到磁盘后原子替换；失败时保留原文件 |
...
**状态：待合入（2026-08-15 正式确定；P4.1 至 P4.3 已实现）**
...
| 切片 | 主题 | 状态 | 目标 |
| --- | --- | --- | --- |
| P4.1 | 首次启动 | 待合入 | 从启动窗口选择数据目录，创建默认配置和空 Repository |
| P4.2 | 路径修复与启动诊断 | 待合入 | 区分配置、数据目录和 Repository 故障；验证后修复数据路径，并支持修改每个索引的本地目录 |
| P4.3 | 单个索引故障隔离 | 待合入 | 跳过缺失、损坏或不可读的单个 File Tree，加载其余会话并汇总警告 |
